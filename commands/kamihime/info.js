...
class InfoCommand extends Command {
    ...
    try {
      ...
      switch(true) {
        ...
        case category.includes('Category:Weapons'):
          embed = this.weaponTemplate(result, dbRes, prefix);
          break;
        case category.includes('Category:Accessories'):
          embed = this.accessoryTemplate(result, dbRes, prefix);
          break;
  ...
  kamihimeTemplate(result, dbRes, prefix) {
    const hime = {
      ...
      harem: dbRes.khHarem_hentai1Resource2 || dbRes.khHarem_hentai2Resource2 ? true : false
    };
    const embed = this.client.util.embed()
  ...
  weaponTemplate(result, dbRes, prefix) {
    const weapon = {
      name: result.name,
      description: result.description,
      link: `${this.wikiaURL}${encodeURI(result.name)}`,
